...
class EarlyStopping:
    ...
    def save_checkpoint(self, encoder, decoder):
        torch.save(encoder.state_dict(), os.path.join(self.path, "encoder_best.pt"))
        torch.save(decoder.state_dict(), os.path.join(self.path, "decoder_best.pt"))
        if self.verbose:
            print(f"✅ Validation loss improved. Saved best model to '{self.path}'")