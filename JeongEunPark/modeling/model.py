...
# nn.Sequential : 순차적인 레이어를 쉽게 쌓을 수 있도록 해주는 클래스
# nn.Linear : Fully Connected Layer을 구현하는 클래스, 선형 변환을 수행하는 레이어, 입력값과 
# 가중치를 곱하고, 편향을 더하는 연산을 함 y = Wx + b 형태의 선형 연산을 기반으로 함
# 이미지 -> 임베딩(KoGPT2입력에 맞춰서)
import torch
import torch.nn as nn
import torchvision.models as models
from transformers import GPT2LMHeadModel, PreTrainedTokenizerFast
...
#     def __init__(self, embed_size):
#         super(VisionEncoder, self).__init__() # 부모 클래스를 초기화한 후, 모델의 레이어와 파라미터를 초기화
#         # 사전학습된 ResNet50 불러오기
#         self.resnet = models.resnet50(pretrained=True)
#         self.resnet_dim = 2048
#         self.resnet.fc = nn.Identity() # 마지막 분류 레이어 제거
#         self.vision_projection = nn.Sequential(
#             nn.Linear(self.resnet_dim, embed_size),
#             nn.ReLU(),
#             nn.Dropout(0.1),
#             nn.Linear(embed_size, embed_size)
#         )
#         self.feature_expansion = nn.Sequential( # ResNet feature을 여러 토큰으로 확장하기 위한 레이어
#             nn.Linear(embed_size, embed_size * 49),
#             nn.ReLU(),
#             nn.Dropout(0.1)
#         )
#         # self.linear = nn.Linear(resnet.fc.in_features, embed_size) # [B, 2048]벡터를 embed_size로 줄이는 선형 변환 레이어
...
#     def forward(self, images): # 입력 데이터에 대한 연산을 정의 < 여기서 images는 PIL 이미지가 아니라 이미 전처리된 pytorch Tensor
#         features = self.resnet(images) # [B, 2048]
#         features = self.vision_projection(feature) # [B, 768]
#         expanded_features = self.feature_expansion(features) # [B, 768 * 49]
#         features = expanded_features.view(expanded_features.size(0), 49, -1) #[B, 49, 768]
